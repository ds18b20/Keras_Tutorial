...
class YOLO(object):
    def __init__(self, obj_threshold, nms_threshold):
        ...
        self._yolo = load_model('data/yolov3.h5')
    ...
    def _process_feats(self, out, anchors, mask):
        ...
        out = out[0]  # only a single batch: batch_0
        """
        out structure:
        [tx, ty, tw, th, p0, p1, p2, p3...pC]
        Box Coordinates: [tx, ty, tw, th]
        Objectness Score: p0
        Class Scores: p1...pC
        """
        box_xy = self._sigmoid(out[..., :2])  # σ(tx, ty)
        box_wh = np.exp(out[..., 2:4])  # exp(tw, th)
        print("box_wh.shape:", box_wh.shape)
        box_wh = box_wh * anchors_tensor

        box_confidence = self._sigmoid(out[..., 4])  # => np.ndarray
        box_confidence = np.expand_dims(box_confidence, axis=-1)  # objectness prob
        print("box_confidence.shape:", box_confidence.shape)
        box_class_probs = self._sigmoid(out[..., 5:])  # class probs
    ...
    def _yolo_out(self, outs, shape):
        ...
        """
        shape of outs:
        out[0].shape: (1, 13, 13, 3, 85)
        out[1].shape: (1, 26, 26, 3, 85)
        out[2].shape: (1, 52, 52, 3, 85)
        """
        """
        anchors for 13x13: [116, 90], [156, 198], [373, 326]
        anchors for 26x26: [30, 61], [62, 45], [59, 119]
        anchors for 52x52: [10, 13], [16, 30], [33, 23]
        """
    ...
    def predict(self, image, shape):
        ...
        for idx, out in enumerate(outs):
            print("out[{0}].shape: {1}".format(idx, out.shape))
